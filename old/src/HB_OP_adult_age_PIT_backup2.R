...
## where the "0" subscript denotes the redundant, top-level parameterization:
## 
##   a = (a0 - a0.int)/sigma
##   b = (b0 - a0.int)/sigma
...
model
{
  ...
  alpha.mu[1] <- sum(X[1,1:numX]*a)                  # expected alpha for year 1
  alpha.yr[1] ~ dnorm(alpha.mu[1], pow(sigma.yr,-2)) # alpha for year 1
  alpha.err[1] <- alpha.yr[1] - alpha.mu[1]          # alpha "residual" for year 1
  
  # partition alpha0 using cutpoints
  F.yr[1,1] <- 0
  for(j in 1:(M-1))                                   
  {
    F.yr[1,j+1] <- phi(tau[j] - alpha.yr[1])
  ...
  for(y in 2:Y)
  {
    alpha.mu[y] <- sum(X[y,1:numX]*a)                # expected alpha for year y
    alpha.yr[y] ~ dnorm(alpha.mu[y] + rho*alpha.err[y-1], pow(sigma.yr,-2))   # alpha for year y
    alpha.err[y] <- alpha.yr[y] - alpha.mu[y]        # alpha "residual" for year y
    
    # partition alpha using cutpoints
    F.yr[y,1] <- 0
    for(j in 1:(M-1))                                   
    {
      F.yr[y,j+1] <- phi(tau[j] - alpha.yr[y])
  ...
  {
    alpha[i] <- alpha.yr[year[i]] + sum(Z[i,1:numZ]*b)     # alpha for individual i
    
    # partition alpha using cutpoints
    F[i,1] <- 0
    for(j in 1:(M-1))                                   
    {
      F[i,j+1] <- phi(tau[j] - alpha[i])
