...
model
{
  ...
  sigma.yr ~ dunif(0,10)    # process error SD
  ...
  alpha0.yr[1] ~ dnorm(alpha0.mu[1], pow(sigma,-2))  # alpha0 for year 1
  alpha.yr[1] <- (alpha0.yr[1] - a0.int)/sigma       # rescaled (identified) alpha for year 1
  ...
  for(j in 1:(M-1))                                   
  {
    F.yr[1,j+1] <- phi(tau0[j] - alpha0.yr[1])
  ...
  for(y in 2:Y)
  {
    ...
    alpha0.yr[y] ~ dnorm(alpha0.mu[y] + rho*alpha0.err[y-1], pow(sigma,-2))   # alpha for year y
    alpha.yr[y] <- (alpha0.yr[y] - a0.int)/sigma     # rescaled (identified) alpha for year 1
    ...
    for(j in 1:(M-1))                                   
    {
      F.yr[y,j+1] <- phi(tau0[j] - alpha0.yr[y])
  ...
  {
    alpha0[i] <- alpha0.yr[year[i]] + sum(Z[i,1:numZ]*b0)  # alpha0 for individual i
    
    # partition alpha0 using cutpoints
    F[i,1] <- 0
    for(j in 1:(M-1))                                   
    {
      F[i,j+1] <- phi(tau0[j] - alpha0[i])
